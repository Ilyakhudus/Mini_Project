...
const votePoll = async (req, res) => {
  try {
    ...
    // If this poll message references a Poll document, ensure poll is active before accepting votes
    if (message.pollId) {
      try {
        const poll = await Poll.findById(message.pollId)
        if (!poll) {
          return res.status(404).json({ error: "Associated poll not found" })
        }
        if (!poll.isActive) {
          return res.status(400).json({ error: "Poll is closed" })
        }
      } catch (err) {
        console.error("Error checking associated poll status:", err)
    ...
    // If this poll message references a Poll document, sync votes -> Poll.responses and option counts
    if (message.pollId) {
      try {
        const poll = await Poll.findById(message.pollId)
        if (poll) {
          // Rebuild poll.responses from message.pollOptions
          poll.responses = []
          // Reset numeric vote counts
          ...
          message.pollOptions.forEach((opt, idx) => {
            (opt.votes || []).forEach((v) => {
              poll.responses.push({
                userId: v.user,
                selectedOption: idx,
                responseTime: v.votedAt || Date.now(),
              })
              if (poll.options[idx]) {
        ...
        console.error("Failed to sync poll with message votes:", err)
      }
    }
