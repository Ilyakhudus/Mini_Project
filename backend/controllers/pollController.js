...
exports.createPoll = async (req, res, next) => {
  try {
    ...
    // Broadcast poll to registered attendees as a message in the updates panel
    try {
      const registrations = await Registration.find({ event: eventId })
      const recipients = registrations.map((reg) => ({ user: reg.user, read: false }))

      const messageData = {
        event: eventId,
        sender: req.user.id,
        title: `Poll: ${poll.question}`,
        content: poll.description || poll.question,
        messageType: "poll",
        pollOptions: poll.options.map((opt) => ({ text: opt.optionText, votes: [] })),
      ...
      console.error("Failed to broadcast poll message:", err)
    }
